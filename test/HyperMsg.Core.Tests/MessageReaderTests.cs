...
namespace HyperMsg.Transciever
{
    public class MessageReaderTests
    {
        ...
        {
            var reader = new MessageReader<Guid>(b => new DeserializationResult<Guid>(0, Guid.Empty));
            var handler = A.Fake<Action<Guid>>();
            reader.SetMessageHandler(handler);
        ...
        public void ReadBuffer_Calls_Handler_For_Deserialized_Message()
        {
            ...
            var reader = new MessageReader<Guid>(b => new DeserializationResult<Guid>(byteCount, message));
            var handler = A.Fake<Action<Guid>>();
            reader.SetMessageHandler(handler);
        ...
        {
            var reader = new MessageReader<Guid>(b => new DeserializationResult<Guid>(-1, default));
