...
namespace HyperMsg
{
    public static class MessagingContextExtensions
    {
        ...
        public static Task<TResponse> SendAndWaitMessage<TRequest, TResponse>(this IMessagingContext messagingContext, TRequest request, Func<TResponse, bool> responsePredicate, CancellationToken cancellationToken = default) => 
            messagingContext.SendAndWaitMessage<TRequest, TResponse>((sender, token) => sender.SendAsync(request, token), responsePredicate, cancellationToken);

        public static async Task<TResponse> SendAndWaitMessage<TRequest, TResponse>(this IMessagingContext messagingContext, AsyncAction<IMessageSender> sendAction, Func<TResponse, bool> responsePredicate, CancellationToken cancellationToken = default)
        {
